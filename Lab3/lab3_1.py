...
def hack_caesar_by_freq(enc_letters: str):
    ...
    # while any(map(lambda it: abs(it[0] - it[1]) > ABC_FREQ_E,
    #               zip(typic_freq, enc_letters_freq[offset:] + enc_letters_freq[:offset]))) and offset < ABC_LENGTH:
    #     offset += 1

    for o in range(ABC_LENGTH):
        offset = o
        enc_offseted = enc_letters_freq[offset:] + enc_letters_freq[:offset]
        is_finded = False
        for index in range(ABC_LENGTH):
            if abs(typic_freq[index] - enc_offseted[index]) > ABC_FREQ_E:
                is_finded = True
                break
        if is_finded:
            break
